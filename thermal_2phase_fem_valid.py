...
import config_2phase_valid as cfg

if autogen_output_dir:
    output_dir = os.path.join("./output/thermal_2phase_1D_valid", "T_evolution")
...
Kt_vals = np.zeros(2*n_elem*stride)
Kt_rows = np.zeros(2*n_elem*stride, dtype=np.int32)
Kt_cols = np.zeros(2*n_elem*stride, dtype=np.int32)
...
for i_elem in range(n_elem):
    ...
    K_vals[iptr_slice] = (Kuu0 + Kuu2).flatten()
    ...
    Kt_vals[(2*i_elem)*stride: (2*i_elem+1)*stride] = Kuu1.flatten()
    Kt_rows[(2*i_elem)*stride: (2*i_elem+1)*stride] = i_rows.flatten()
    Kt_cols[(2*i_elem)*stride: (2*i_elem+1)*stride] = i_cols.flatten()
...
Kt_mat = sparse.coo_array((Kt_vals, (Kt_rows, Kt_cols)), shape=(n_dof, n_dof)).tocsr()
...
# print(Kbb2, Kbb1, Kbb0, sep='\n')
# print(np.linalg.cond(M_mat.todense()))
# print((K_mat + Kt_mat) @ np.ones(n_dof))
...
# Initial condition
solution_array[0, :] = cfg.T0

for t_id in range(t_array.size - 1):
    flow = cfg.get_background_flow(t_array[t_id])
    
    if time_stepper == "implicit":
        L_mat = M_mat - dt*(K_mat + flow*Kt_mat)
        f_vec = M_mat @ solution_array[t_id]
        
        # Boundary conditions
        if np.allclose(flow, 1.):
            solution_array[t_id+1, 0] = cfg.Th
            f_vec -= L_mat @ solution_array[t_id+1, :]
            L_mat = L_mat[1:, 1:]
            f_vec = f_vec[1:]
            idx_rem = [i for i in range(n_dof) if i != 0]
        elif np.allclose(flow, -1.):
            solution_array[t_id+1, -2] = cfg.Tc
            f_vec -= L_mat @ solution_array[t_id+1, :]
            L_mat = L_mat[:-1, :-1]
            f_vec = f_vec[:-1]
            idx_rem = [i for i in range(n_dof) if i != n_dof-2]
        else:
            idx_rem = np.arange(n_dof)
            
        solution_step = splinalg.spsolve(L_mat, f_vec)
        solution_array[t_id+1, idx_rem] = solution_step
        
        if (t_id + 1) % step_verbose == 0:
            print("{:d}/{:d} steps solved.".format(t_id + 1, t_array.size))
    
    elif time_stepper == "explicit":
        
        f_vec = (K_mat + flow*Kt_mat) @ solution_array[t_id]
        solution_step = splinalg.spsolve(M_mat, f_vec)
        solution_array[t_id+1, :] = solution_array[t_id, :] + dt*solution_step
        if np.allclose(flow, 1.):
            solution_array[t_id+1, 0] = cfg.Th
        elif np.allclose(flow, -1.):
            solution_array[t_id+1, -2] = cfg.Tc
        ...
        print("Invalid time stepper {:s}!".format(time_stepper))
...
if output:
    ...
    save_fname = "solution_fem{:d}_explicit.h5".format(n_elem)
